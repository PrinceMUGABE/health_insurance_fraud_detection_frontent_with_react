...
function EmployeeViewPredictions() {
  ...
  const handleFetch = async () => {
    try {
      const res = await axios.get(
        "http://127.0.0.1:8000/prediction/predictions/"
      );
      if (res.data) {
  ...
  const filteredData = Array.isArray(institutionData)
    ? institutionData.filter(
        (institution) =>
          (filter === "all" ||
            institution.type?.toLowerCase() === filter.toLowerCase()) &&
          (institution.first_name
            .toLowerCase()
            ...
            (institution.insurance &&
              institution.insurance.insurance_code
                .toLowerCase()
                .includes(search.toLowerCase())) ||
    ...
    if (conf) {
      try {
        const res = await axios.delete(
          "http://127.0.0.1:8000/prediction/delete/" + id
        );
        if (res.status === 204) {
          alert("Institution deleted successfully");
          setInstitutionData((prevInstitutionData) =>
            prevInstitutionData.filter((institution) => institution.id !== id)
          );
        } else {
  ...
  return (
    <>
      <h1 className="text-center font-bold text-black text-xl capitalize mb-4 mt-3 pb-4">
        Insurance Prediction Results
      </h1>
      ...
      <div className="flex justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <option className="text-gray-600" value="asc">
              Available
            </option>
            <option className="text-gray-600" value="desc">
              Frauded
            </option>
          </select>
        </div>
        <Link
          to="/employee/predictInsurance"
        ...
        <button
          ...
          className="px-4 py-2 bg-green-400 text-white rounded-full hover:bg-gray-500 hover:text-white"
        >
          Download PDF
        </button>
        <button
          onClick={handleExcelDownload}
          className="px-4 py-2 bg-blue-400 text-white rounded-full hover:bg-gray-500 hover:text-white"
        >
          Download Excel
        </button>
      </div>
